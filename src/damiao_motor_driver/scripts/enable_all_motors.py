...
def make_command_frame(cmd_id, op_type, motor_id=0xFF):
    ...
    Args:
        cmd_id: 命令ID (0x01-0x04)
        op_type: 操作类型 (0x00=单电机, 0xFF=所有电机)
        motor_id: 电机ID (单电机时有效, 0xFF表示所有电机)
...
def parse_response_frame(data):
    ...
    Returns:
        (cmd_id, status, op_type, motor_id) 或 None
    """
...
def send_command(ser, cmd_id, op_type=OP_TYPE_ALL, motor_id=0xFF):
    ...
    Returns:
        (success, status_code, response_data) 或 (False, None, None)
    """
    # 构造命令帧
