...
def get_status_name(status):
    ...
    return status_names.get(status, f"未知错误(0x{status:02X})")
    ...
    try:
        while True:
            ...
            if choice == '0':
                ...
                print("\n发送使能所有电机命令...")
                success, status, response = send_command(ser, CMD_ENABLE_MOTOR, OP_TYPE_ALL, 0xFF)
                if success:
                    print("✓ 所有电机已使能")
                else:
                    print(f"✗ 失败: {get_status_name(status) if status else '未收到响应'}")
            
            elif choice == '2':
                print("\n发送失能所有电机命令...")
                success, status, response = send_command(ser, CMD_DISABLE_MOTOR, OP_TYPE_ALL, 0xFF)
                if success:
                    print("✓ 所有电机已失能")
                else:
                    print(f"✗ 失败: {get_status_name(status) if status else '未收到响应'}")
                ...
                print("\n发送清除所有电机错误命令...")
                success, status, response = send_command(ser, CMD_CLEAR_ERROR, OP_TYPE_ALL, 0xFF)
                if success:
                    print("✓ 所有电机错误已清除")
                else:
                    print(f"✗ 失败: {get_status_name(status) if status else '未收到响应'}")
                ...
                motor_id = int(input("请输入电机ID (1-30): "))
                if 1 <= motor_id <= 30:
                    print(f"\n发送使能电机 {motor_id} 命令...")
                    success, status, response = send_command(ser, CMD_ENABLE_MOTOR, OP_TYPE_SINGLE, motor_id)
                    if success:
                        print(f"✓ 电机 {motor_id} 已使能")
                    else:
                        print(f"✗ 失败: {get_status_name(status) if status else '未收到响应'}")
                else:
                    print("无效的电机ID，范围应为 1-30")
            
            elif choice == '6':
                motor_id = int(input("请输入电机ID (1-30): "))
                if 1 <= motor_id <= 30:
                    print(f"\n发送失能电机 {motor_id} 命令...")
                    success, status, response = send_command(ser, CMD_DISABLE_MOTOR, OP_TYPE_SINGLE, motor_id)
                    if success:
                ...
                motor_id = int(input("请输入电机ID (1-30): "))
                if 1 <= motor_id <= 30:
                    print(f"\n警告：此操作会将电机 {motor_id} 的当前位置设置为零点！")
                    confirm = input("确认继续？(y/n): ")
                    if confirm.lower() == 'y':
                        print(f"\n发送保存电机 {motor_id} 位置零点命令...")
                        success, status, response = send_command(ser, CMD_SAVE_ZERO, OP_TYPE_SINGLE, motor_id)
                        if success:
                            print(f"✓ 电机 {motor_id} 位置零点已保存")
                        else:
                            print(f"✗ 失败: {get_status_name(status) if status else '未收到响应'}")
                    else:
                        print("操作已取消")
                ...
                motor_id = int(input("请输入电机ID (1-30): "))
                if 1 <= motor_id <= 30:
                    print(f"\n发送清除电机 {motor_id} 错误命令...")
                    success, status, response = send_command(ser, CMD_CLEAR_ERROR, OP_TYPE_SINGLE, motor_id)
                    if success:
                        print(f"✓ 电机 {motor_id} 错误已清除")
                    else:
                        print(f"✗ 失败: {get_status_name(status) if status else '未收到响应'}")
                else:
                    print("无效的电机ID，范围应为 1-30")
