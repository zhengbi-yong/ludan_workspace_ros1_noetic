...
private:
  ...
  // 启动阶段保持当前姿态，待收到首个命令后再释放
  std::atomic<bool> hold_position_mode_{true};
  std::atomic<bool> received_first_state_{false};
  std::atomic<bool> hold_notice_printed_{false};
  std::atomic<bool> hold_release_printed_{false};
  std::atomic<bool> hold_position_synced_{false};
  const double hold_position_tolerance_{1e-3};
};
